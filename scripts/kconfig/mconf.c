...
static const char
    ...
    inputbox_instructions_int[] =
        N_("Please enter a decimal value. "
           ...
           "Use the <TAB> key to move from the input field to the buttons "
           "below it."),
    ...
    inputbox_instructions_string[] =
        N_("Please enter a string value. "
           ...
           "below it."),
    setmod_text[] =
        N_("This feature depends on another which has been configured as a "
           "module.\n"
...
static struct gstr get_relations_str(struct symbol **sym_arr) {
    ...
    if (!i) str_append(&res, "No matches found.\n");
    return res;
}
...
static void conf(struct menu *menu) {
    ...
    while (1) {
        ...
        for (i = 0; input_buf[i] && !isspace(input_buf[i]); i++)
            ;
        if (i >= sizeof(active_entry)) i = sizeof(active_entry) - 1;
        input_buf[i] = 0;
        strcpy(active_entry, input_buf);

        sym = NULL;
        submenu = NULL;
        if (sscanf(input_buf + 1, "%p", &submenu) == 1) sym = submenu->sym;
...
static void conf_choice(struct menu *menu) {
    ...
    while (1) {
        ...
        switch (stat) {
            case 0:
                if (sscanf(input_buf, "%p", &child) != 1) break;
...
static void conf_string(struct menu *menu) {
    ...
    while (1) {
        ...
        switch (stat) {
            case 0:
                if (sym_set_string_value(menu->sym, input_buf)) return;
...
static void conf_load(void) {
    ...
    while (1) {
        ...
        switch (stat) {
            case 0:
                if (!input_buf[0]) return;
                if (!conf_read(input_buf)) return;
...
static void conf_save(void) {
    ...
    while (1) {
        ...
        switch (stat) {
            case 0:
                if (!input_buf[0]) return;
                if (!conf_write(input_buf)) return;
                show_textbox(
                    NULL,
